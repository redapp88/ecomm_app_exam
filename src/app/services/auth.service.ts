...
export class AuthService implements OnDestroy {
    ...
    public  subscribe(username: string, password: string, firstName: string,lastName: string,adresse: string,mail: string,city: string, phone: string) {
        return  this.http.post
        (`${environment.backEndUrl}/subscribe`,{username:username,password:password,firstName:firstName,lastName:lastName,adresse:adresse,mail:mail,city:city,phone:phone})
    }
        ...
        if(jwt){
            let user:AppUser=
                new AppUser(jwtHelper.decodeToken(jwt).sub,
                    ...
                    jwtHelper.decodeToken(jwt).adress,
                    jwtHelper.decodeToken(jwt).city,
            ...
            Plugins.Storage.get({key:'authData'}).then(
                (authData=>{
                    if(!authData || !authData.value){
                        ...
                        let user=
                            new AppUser(data.username,data.firstName,data.lastName,data.adress,data.city,data.state,data.roles,new Date(data.expirationDate),data.jwt)
